...
export const PivotPanel: React.FC<PivotPanelProps> = ({ currentContext, onPivot }) => {
  ...
  return (
    <div className="w-full bg-zinc-900/50 text-white p-6 md:p-8">
      <div className="container mx-auto">
        
        {/* Header - Stays at top */}
        <div className="mb-6 border-b border-zinc-800 pb-4">
          <h3 className="text-xl font-bold flex items-center gap-2 mb-1 text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          {/* Column 1: Active Context */}
          <div className="space-y-4">
            <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-500">Active Context</h4>
            <div className="bg-black/60 border border-zinc-800 rounded-xl p-5 space-y-3">
              ...
              <div className="flex flex-col gap-1 pt-3 border-t border-zinc-800 mt-2">
          ...
          {/* Column 2: The Input Engine */}
          <div className="space-y-3">
             <label className="text-xs font-bold uppercase tracking-wider text-zinc-500">Natural Language Pivot</label>
             <textarea 
               className="w-full h-32 bg-black border border-zinc-700 rounded-xl p-4 text-sm text-white focus:ring-1 focus:ring-blackline-yellow outline-none resize-none placeholder-zinc-600"
             ...
             >
               {isProcessing ? <RubiksCube size={16} className="animate-spin"/> : <Sparkles size={16} />}
               {isProcessing ? 'Pivoting...' : 'Pivot Now'}
             </button>

             {/* Feedback Toast embedded in column */}
             {lastPivot && (
          ...
          <div className="space-y-3">
             <p className="text-xs font-bold uppercase tracking-wider text-zinc-500">Quick Pivots</p>
             <div className="flex flex-col gap-2">
                <button onClick={() => setPrompt("Pivot to Healthcare CFO worried about regulatory fines")} className="w-full text-left text-xs text-gray-400 hover:text-blackline-yellow hover:bg-zinc-800 p-3 rounded-lg border border-zinc-800 transition-colors">
                   "Pivot to Healthcare CFO worried about fines"
                </button>
                <button onClick={() => setPrompt("Retail company $10B revenue, focus on high turnover and peak cycle burnout")} className="w-full text-left text-xs text-gray-400 hover:text-blackline-yellow hover:bg-zinc-800 p-3 rounded-lg border border-zinc-800 transition-colors">
                   "Retail co. $10B revenue, focus on burnout"
                </button>
                <button onClick={() => setPrompt("Tech SaaS, Pre-IPO, focus on ASC 606 compliance")} className="w-full text-left text-xs text-gray-400 hover:text-blackline-yellow hover:bg-zinc-800 p-3 rounded-lg border border-zinc-800 transition-colors">
                   "Tech SaaS, Pre-IPO, focus on ASC 606"
                </button>
             </div>
          </div>
