...
// --- SAFE ICON COMPONENT ---
// Prevents crashes if an icon name in constants.ts doesn't match Lucide exactly
const SafeIcon = ({ name, className }: { name: string; className?: string }) => {
  const IconComponent = (Icons as any)[name];
  
  if (!IconComponent) {
    // Fallback mapping for specific hardcoded names if dynamic lookup fails
    switch (name) {
...
export const PlatformHub: React.FC<PlatformHubProps> = ({
  ...
  // Group Personas dynamically from Constants (The 7 you requested)
  const executivePersonas = PERSONAS.filter(p => p.group === 'Executive');
  const operationalPersonas = PERSONAS.filter(p => p.group === 'Operational');

  // Helper to check if a driver is selected
    ...
    <div className="w-full max-w-6xl mx-auto px-4 md:px-6 pb-32 animate-fade-in">
      ...
      <div className="space-y-6">
        ...
        <div className={`bg-zinc-900 border ${openSection === 'persona' ? 'border-blackline-yellow' : 'border-zinc-800'} rounded-3xl overflow-hidden transition-all duration-300`}>
           <button 
              ...
              className="w-full flex items-center justify-between p-6 md:p-8 bg-zinc-950 hover:bg-zinc-900 transition-colors"
           >
              <div className="flex items-center gap-4">
                 <div className={`w-10 h-10 rounded-full flex items-center justify-center ${selectedPersona ? 'bg-blackline-yellow text-black' : 'bg-zinc-800 text-zinc-500'}`}>
                    <Users size={20} />
                 </div>
                 <div className="text-left">
                    <h3 className="text-xl font-bold text-white">Select Persona</h3>
                    <p className="text-sm text-zinc-400">
                       {selectedPersona ? selectedPersona.name : "Who is this analysis for?"}
                    </p>
                 </div>
              </div>
           ...
           {openSection === 'persona' && (
              <div className="p-6 md:p-8 border-t border-zinc-800 animate-slide-down">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                       {executivePersonas.map((persona) => (
                          <button
                             ...
                             className={`p-6 rounded-2xl border text-left transition-all group relative overflow-hidden ${selectedPersona?.id === persona.id ? 'bg-zinc-800 border-blackline-yellow shadow-lg' : 'bg-black/40 border-zinc-800 hover:border-zinc-600'}`}
                          >
                             <div className="mb-4">
                                <SafeIcon name={persona.icon} className={`w-8 h-8 ${selectedPersona?.id === persona.id ? 'text-blackline-yellow' : 'text-zinc-400 group-hover:text-white'}`} />
                             </div>
                             <span className={`text-sm font-bold uppercase tracking-wider ${selectedPersona?.id === persona.id ? 'text-white' : 'text-zinc-400 group-hover:text-white'}`}>
                                {persona.name}
                             </span>
                             {selectedPersona?.id === persona.id && (
                                <div className="absolute top-4 right-4 text-blackline-yellow">
                                   <CheckCircle size={18} />
                                </div>
                             )}
                          </button>
                 ...
                 {/* Operational Tier */}
                 <div>
                    <h4 className="text-xs font-black text-zinc-500 uppercase tracking-widest mb-4">Tactical Operational</h4>
        ...
        <div className={`bg-zinc-900 border ${openSection === 'industry' ? 'border-blackline-yellow' : 'border-zinc-800'} rounded-3xl overflow-hidden transition-all duration-300`}>
           ...
           >
              <div className="flex items-center gap-4">
                 <div className={`w-10 h-10 rounded-full flex items-center justify-center ${selectedIndustry ? 'bg-blue-500 text-white' : 'bg-zinc-800 text-zinc-500'}`}>
                    <Globe size={20} />
                 </div>
                 <div className="text-left">
                    <h3 className="text-xl font-bold text-white">Select Industry</h3>
                    <p className="text-sm text-zinc-400">
                       {selectedIndustry ? INDUSTRIES.find(i => i.id === selectedIndustry)?.nameKey || selectedIndustry : "Benchmark against peers"}
                    </p>
                 </div>
              </div>
              ...
              <div className="p-6 md:p-8 border-t border-zinc-800 grid grid-cols-2 md:grid-cols-4 gap-4 animate-slide-down">
                 {INDUSTRIES.map((ind) => (
                    <button
                       ...
                       className={`p-4 rounded-xl border text-center transition-all ${selectedIndustry === ind.id ? 'bg-blue-900/30 border-blue-500 text-white' : 'bg-black/40 border-zinc-800 text-zinc-400 hover:border-zinc-600'}`}
                    >
        ...
        <div className={`bg-zinc-900 border ${openSection === 'drivers' ? 'border-blackline-yellow' : 'border-zinc-800'} rounded-3xl overflow-hidden transition-all duration-300`}>
           ...
           >
              <div className="flex items-center gap-4">
                 <div className={`w-10 h-10 rounded-full flex items-center justify-center ${selectedDrivers.length > 0 ? 'bg-green-500 text-white' : 'bg-zinc-800 text-zinc-500'}`}>
                    <Zap size={20} />
                 </div>
                 <div className="text-left">
                    <h3 className="text-xl font-bold text-white">Strategic Drivers</h3>
                    <p className="text-sm text-zinc-400">
                       {selectedDrivers.length} priorities selected
                    </p>
                 </div>
              </div>
           ...
           {openSection === 'drivers' && (
              <div className="p-6 md:p-8 border-t border-zinc-800 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 animate-slide-down">
                 {VALUE_DRIVERS_SELECTION.map((driver) => (
                    <button
                       key={driver.id}
                       onClick={() => onDriverToggle(driver)}
                       className={`flex items-center justify-between p-4 rounded-xl border transition-all ${isDriverSelected(driver.id) ? 'bg-green-900/20 border-green-500 text-white' : 'bg-black/40 border-zinc-800 text-zinc-400 hover:border-zinc-600'}`}
                    >
      ...
      <div className="fixed bottom-0 left-0 w-full bg-black/80 backdrop-blur-xl border-t border-zinc-800 p-4 z-50">
         <div className="max-w-6xl mx-auto flex items-center justify-between">
            ...
            <button 
               onClick={onAnalyze}
               disabled={!selectedPersona || selectedDrivers.length === 0}
               className={`flex items-center gap-3 px-8 py-4 rounded-xl font-black uppercase tracking-wider shadow-xl transition-all ${(!selectedPersona || selectedDrivers.length === 0) ? 'bg-zinc-800 text-zinc-500 cursor-not-allowed' : 'bg-blackline-yellow text-black hover:scale-105 hover:shadow-2xl hover:shadow-blackline-yellow/20'}`}
            >
               {!selectedPersona ? (
                  <>Select Persona <AlertCircle size={18} /></>
               ) : selectedDrivers.length === 0 ? (
                  <>Select Drivers <AlertCircle size={18} /></>
               ) : (
