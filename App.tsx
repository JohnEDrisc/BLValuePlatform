...
function App() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Reduced threshold to 10px so it hides immediately when moving away from top
  ...
  // --- UPDATED HELPER: LARGE WIP BANNER ---
  const WipBanner = ({ title }: { title: string }) => (
    <div className="bg-zinc-900/80 border-2 border-yellow-500/50 rounded-2xl p-8 mb-12 flex flex-col md:flex-row items-center justify-center gap-6 shadow-[0_0_30px_rgba(234,179,8,0.2)] text-center md:text-left mx-auto max-w-4xl">
      ...
      <div>
        <h3 className="text-2xl md:text-3xl font-black text-white uppercase italic tracking-tighter mb-1">
          <span className="text-yellow-500">Work In Progress</span>
        </h3>
        <p className="text-zinc-400 text-sm font-bold uppercase tracking-widest">
          {title} • Internal Beta Release • Do Not Distribute
        </p>
      </div>
    </div>
  );
  ...
  const renderValueNarratives = () => {
    ...
    return (
      <div className="max-w-4xl mx-auto pb-40"> 
        {/* ADDED WIP BANNER TO VALUE NARRATIVES LANDING */}
        <WipBanner title="Value Narratives" />
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 font-sans selection:bg-blackline-yellow selection:text-black pb-20">
      ...
      {/* Navigation Rail */}
      <div 
        className={`fixed bottom-6 left-0 w-full flex justify-center z-[100] transition-all duration-500 no-print ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-24 opacity-0'}`}
            ...
            <>
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2.5 md:px-6 md:py-3 text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] rounded-xl transition-all border whitespace-nowrap
                    ${(activeTab === item.id && !showSkoExplainer) || (showSkoExplainer && item.id === 'sko') ? 'bg-blackline-yellow text-black border-blackline-yellow shadow-[0_0_20px_rgba(249,183,52,0.3)] scale-105' : 'bg-transparent text-gray-400 border-transparent hover:border-zinc-700 hover:text-white hover:bg-zinc-800'}
                  `}
                >
                  <item.icon size={14} />
                  {item.label}
              ...
              <div className="px-4 py-2 hidden lg:flex flex-col items-start min-w-[120px]">
                  <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">{UI_STRINGS['EN'].footer_internal}</span>
              </div>
            </>
          )}
